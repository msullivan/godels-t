...
module HT where
  ...
  HT nat e = HTω e
  HT (A ⇒ B) e = (e' : TCExp A) → HT A e' → HT B (e $ e')
  ...
  head-expansion {A ⇒ B} eval ht-logic =
  ...
  -- Special case of above for closed terms
  all-closed-HT : ∀{A} → (e : TCExp A) → HT A e
  all-closed-HT {A} e = ID.coe1 (HT A) (subid e) (all-HT e (emptyHTΓ {emptyγ}))
  ...
  inhabitant : (A : TTp) → TCExp A
  inhabitant nat = zero
  inhabitant (A ⇒ B) = Λ (ren bsRename (inhabitant B))
  ...
  HT-halts {A ⇒ B} e f with inhabitant A
  ... | e' = lhs-halt (HT-halts (e $ e') (f e' (all-closed-HT e')))


  -- Prove that all programs in Gödel's System T halt.
  all-halt : ∀{A} → (e : TCExp A) → THalts e
  all-halt {A} e = HT-halts e (all-closed-HT e)
